...
class Worker(object):
  ...
  def __init__(self, flags, redis_server, redis_port, model_repo_path, host_repo_path, container_id, worker_name=None, dummy_load=True, *args, **kwargs):
    ...
    if self.dummy_load:
      ...
      models.TFSServerModel.initServer(self.docker_client, self.host_repo_path, flags.worker_memory, self.network_name, arm=flags.use_arm, record=flags.record_measurements)
    common.getModelInfo(self.db, json_file=flags.model_description_file)
  ...
  def processEventQueue(self):
    while True:
      ...
      if (not event.is_model_change):
        ...
        if not self.models_by_name[event.model_name].isLoaded():
          self._unused_queue.put(event)
          continue
      # Check if there is already a model change open
      #if (self.open_model_change and event.is_model_change):
      #  self._unused_queue.put(event)
      #  continue
  ...
  def processEventQueue_new(self):
    while True:
      ...
      if (not event.is_model_change) and (not self.models_by_name[event.model_name].isLoaded()):
  ...
  def loadModel(self, model_name, *args, **kwargs):
    ...
    if model_name not in self.models_by_name:
      raise exceptions.ModelNotInRepoException(f"Model '{model_name}' not available in model repository")
  ...
  def _loadModel(self, model_name):
    ...
    self.open_model_change = True
    if self.flags.load_in_background:
      self.models_by_name[model_name].startModel(model_stats, callback_func=self._mergeQueues)
    else:
  ...
  def _mergeQueues(self):
    logging.info(f"calling _mergeQueues")
    items_merged = 0
    while not self._unused_queue.empty():
      self.event_queue.put(self._unused_queue.get())
      items_merged += 1
    logging.debug(f"merged {items_merged} events")
    self.open_model_change = False
  ...
  def _isThereFreeSpaceToLoad(self, model_name, *args, **kwargs):
    ...
    if self.dummy_load:
      ...
      running_models = [m for m in self.models_by_name.values() if m.isLoaded()]
  ...
  def recordRequestEntry(self, model_requested):
    logging.info(f"recordRequestEntry({model_requested})")
...
def getParser(add_help=True, include_parents=True):
  ...
  parser.add_argument('--record_measurements', action="store_true")
  
  parser.add_argument('--load_in_background', action="store_true", help="If enabled then model loading will be done in separate thread and inferences will be executed.")
...
def main():
  ...
  common.getLogger(hide_debug=False) #f"{os.path.basename(__file__).replace('.py', '')}")
  ...
  logging.info(f"platform.processor(): '{platform.processor()}'")
  
  flags.use_arm = (platform.machine() == 'aarch64')
